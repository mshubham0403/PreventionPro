...
export default function DataEntry({ urlD }) {
  ...
  const [entries, setEntries] = useState([{disease:"",cases:0,diseaseId:"",dateAdded:"",userThatAdded:""}]);
 ...
 setDiseases(prev => (res.data))
  ...
  const handleCasesChange = (index, value) => {
    const newEntries = [...entries];
    newEntries[index].cases = value;
    setEntries(prev => (newEntries));
  };

  const handleSubmit = async(e) => {
    e.preventDefault();
    const diseaseData = {hospitalName:selectedHospital.hospitalName,hospitalId:selectedHospital.hospitalId,arrDiseasesCases:entries,date:selectedDate,user:""}
    //submitting the form data to the server
await axios.post(urlD+"/dailylog",diseaseData)
.then((res)=>{
console.log(res.data);
})
    setEntries(prevEnt=>([{disease:"",cases:0,diseasId:"",dateAdded:""}]))
    console.log(entries);
  };
  ...
  return (
    <div>
      ...
      {selectedHospital && (
        <div>
          <h4 className="text-center mt-3">Selected Hospital: {selectedHospital.hospitalName} with :{selectedHospital.hospitalId}</h4>
            ...
            {entries.map((entry, indexEnt) => (
              <div className='row' key={indexEnt}>
                <label className='col-sm-6'>
                  ...
                  <select
                    value={entry.disease}
                    onChange={(e) => handleDiseaseChange(indexEnt, e.target.value)}
                    required
                  >
                ...
                <label className='col-sm-6'>
                  ...
                  <input
                    type="number"
                    value={entry.cases}
                    onChange={(e) => handleCasesChange(indexEnt, e.target.value)}
                    required
                  />
                </label>
              </div>
            ))}
            <div style={{"marginTop":"30px"}} className="d-flex justify-content-between align-items-center">
              <button className="btn btn-primary" type="button" onClick={handleAddEntry}>
                Add Entry
              </button>
              <button className="btn btn-danger" type="button" onClick={handleDeleteEntry}>
                Delete Last Entry
              </button>
